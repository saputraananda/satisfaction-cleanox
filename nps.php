...
<head>
  ...
  <title>Rekomendasi — Waschen Alora</title>
  ...
  <style>
    * { font-family: 'Poppins', sans-serif; }
    ...
    .nps-btn { transition: transform .15s, box-shadow .15s; user-select: none; cursor: pointer; }
    .nps-btn:hover { transform: translateY(-4px) scale(1.1); }
    .nps-btn.active { transform: translateY(-6px) scale(1.18); box-shadow: 0 8px 20px rgba(0,0,0,.2); }
    .progress-bar { transition: width .5s cubic-bezier(.4,0,.2,1); }
    .btn-primary { background: #5B005F; transition: background .2s, transform .15s; }
    .btn-primary:hover:not(:disabled) { background: #430046; }
    .btn-primary:active:not(:disabled) { transform: scale(0.98); }
    .category-badge { transition: all .3s ease; }
  </style>
</head>
<body class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
  ...
  <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg px-8 py-10 animate-card-in relative z-10">
    ...
    <!-- CSAT recap -->
    <div class="flex items-center gap-3 rounded-2xl px-4 py-3 mb-7 border animate-slide-in" style="background:#F6F1F7;border-color:#E9D5EA;">
      <span class="text-xl"><?= $csat_emoji ?></span>
      <div class="flex-1">
        <p class="text-xs text-gray-400 font-medium">Jawaban CSAT Anda</p>
    ...
    <div class="text-center mb-6 animate-fade-up" style="animation-delay:.08s">
      ...
      <h2 class="text-xl font-bold text-gray-800 leading-snug">
        ...
        <span style="color:#5B005F;">merekomendasikan</span> Waschen Alora<br>
    ...
    <div id="categoryBadge" class="<?= $preselected >= 0 ? '' : 'hidden' ?> category-badge text-center mb-5">
      <span id="categoryInner" class="inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold shadow-sm">
    ...
    <form method="POST" id="npsForm">
      ...
      <div class="grid grid-cols-11 gap-1.5 mb-5" id="npsGrid">
        <?php for ($n = 0; $n <= 10; $n++):
          if ($n <= 6)      { $bg='#FFF5F5';$tc='#EF4444';$bc='#FECACA';$abg='#EF4444';$atc='#fff';$abc='#EF4444'; }
          elseif ($n <= 8)  { $bg='#FFFBEB';$tc='#D97706';$bc='#FDE68A';$abg='#F59E0B';$atc='#fff';$abc='#F59E0B'; }
          else              { $bg='#F0FDF4';$tc='#16A34A';$bc='#BBF7D0';$abg='#10B981';$atc='#fff';$abc='#10B981'; }
        ...
        <button
          ...
          id="npsBtn<?= $n ?>"
          data-score="<?= $n ?>"
          data-bg="<?= $bg ?>" data-tc="<?= $tc ?>" data-bc="<?= $bc ?>"
          data-abg="<?= $abg ?>" data-atc="<?= $atc ?>" data-abc="<?= $abc ?>"
          class="nps-btn aspect-square flex items-center justify-center rounded-xl border-2 text-xs font-bold <?= $isActive ? 'active' : '' ?>"
          style="background:<?= $isActive?$abg:$bg ?>;color:<?= $isActive?$atc:$tc ?>;border-color:<?= $isActive?$abc:$bc ?>;"
        ><?= $n ?></button>
        <?php endfor; ?>
      </div>
      ...
      <div class="flex justify-between text-xs text-gray-400 font-medium mb-6 px-1">
        <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-sm inline-block" style="background:#EF4444;"></span><span>Detractor (0–6)</span></div>
        <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-sm inline-block" style="background:#F59E0B;"></span><span>Passive (7–8)</span></div>
        <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-sm inline-block" style="background:#10B981;"></span><span>Promoter (9–10)</span></div>
      </div>
