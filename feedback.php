...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $allowed_tags = [
    'Edukasi Teknisi',       'Kerapihan Pengerjaan', 'Komunikasi Tim',
    'Respons Admin',         'Penanganan Keluhan',   'Hasil Akhir Pembersihan',
    'Detail Pengerjaan',     'Profesionalitas Tim',  'Ketelitian Kerja',
  ];
  ...
  <style>
    * {
    ...
    textarea:focus {
      ...
      box-shadow: 0 0 0 3px rgba(12, 36, 97, .1);
    }
...
<body class="min-h-screen flex items-start sm:items-center justify-center p-4 py-8 relative">
  ...
  <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg px-8 py-10 animate-card-in relative z-10">
    ...
    <div class="flex gap-3 mb-7 animate-fade-up" style="animation-delay:.05s">
      <div class="flex-1 flex items-center gap-2 rounded-2xl px-3 py-2.5 border"
    ...
    <div class="text-center mb-6 animate-fade-up" style="animation-delay:.1s">
      ...
      <p class="text-gray-400 text-sm mt-1.5">Pilih area masukan <span
          class="font-medium text-gray-500">(opsional)</span></p>
    </div>

    <!-- Error -->
    <?php if ($error): ?>
      <div
        class="mb-4 flex items-center gap-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-2xl text-sm">
    ...
    <form method="POST" id="feedbackForm" autocomplete="off">
      ...
      <div class="flex flex-wrap gap-2 mb-6">
        <?php
        $tags = [
          ['label' => 'Edukasi Teknisi',        'icon' => '📚'],
          ['label' => 'Kerapihan Pengerjaan',   'icon' => '✨'],
          ['label' => 'Komunikasi Tim',         'icon' => '🗣️'],
          ['label' => 'Respons Admin',          'icon' => '💬'],
          ['label' => 'Penanganan Keluhan',     'icon' => '🛠️'],
          ['label' => 'Hasil Akhir Pembersihan','icon' => '🧹'],
          ['label' => 'Detail Pengerjaan',      'icon' => '🔍'],
          ['label' => 'Profesionalitas Tim',    'icon' => '👔'],
          ['label' => 'Ketelitian Kerja',       'icon' => '⭐'],
        ];
        foreach ($tags as $tag):
          $id = 'chip_' . preg_replace('/\W/', '_', $tag['label']);
          ?>
          <label
            class="tag-chip flex items-center gap-1.5 px-3.5 py-2 rounded-full border-2 border-gray-200 bg-white text-gray-600 text-xs font-semibold"
            id="<?= $id ?>">
            <input type="checkbox" name="tags[]" value="<?= htmlspecialchars($tag['label'], ENT_QUOTES) ?>" class="hidden"
      ...
      <div id="tagPreview" class="hidden mb-4 flex items-start gap-2 px-4 py-2.5 rounded-xl text-xs font-medium"
