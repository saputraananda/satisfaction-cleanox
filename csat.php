...
<head>
  ...
  <title>Penilaian Layanan — Waschen Alora</title>
  ...
  <style>
    * { font-family: 'Poppins', sans-serif; }
    ...
    .emoji-btn { transition: transform .2s, box-shadow .2s; cursor: pointer; user-select: none; }
    .emoji-btn:hover { transform: translateY(-5px) scale(1.07); }
    .emoji-btn.active { transform: translateY(-7px) scale(1.14); }
    .emoji-face { transition: transform .2s, filter .2s; display: inline-block; }
    .emoji-btn:hover .emoji-face,
    .emoji-btn.active .emoji-face { transform: scale(1.15); filter: drop-shadow(0 4px 6px rgba(0,0,0,.18)); }
    .progress-bar { transition: width .5s cubic-bezier(.4,0,.2,1); }
    .btn-primary { background: #5B005F; transition: background .2s, transform .15s, opacity .2s; }
    .btn-primary:hover:not(:disabled) { background: #430046; }
...
<body class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
  ...
  <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg px-8 py-10 animate-card-in relative z-10">
    ...
    <div class="flex items-center gap-3 mb-5">
      <a href="index.php?reset=1" class="text-gray-400 hover:text-gray-600 transition-colors p-1" title="Kembali ke awal">
    ...
    <form method="POST" id="csatForm">
      ...
      <div class="grid grid-cols-5 gap-3 mb-8">
        ...
        <button
          type="button"
          onclick="selectCsat(<?= $score ?>)"
          id="csatBtn<?= $score ?>"
          data-score="<?= $score ?>"
          data-default-bg="<?= $opt['bg'] ?>"
          data-default-border="<?= $opt['border'] ?>"
          data-active-bg="<?= $opt['activeBg'] ?>"
          data-active-border="<?= $opt['activeBorder'] ?>"
          ...
          <span class="text-center leading-tight" style="font-size:10px;font-weight:600;color:#475569;"><?= $opt['label'] ?></span>
          <span class="w-2 h-2 rounded-full <?= $isActive ? 'opacity-100' : 'opacity-0' ?> transition-opacity" id="dot<?= $score ?>" style="background:<?= $opt['activeBorder'] ?>;"></span>
        </button>
        <?php endforeach; ?>
      </div>

      <!-- Selected feedback strip -->
      <div id="selectedStrip" class="<?= $preselected ? '' : 'hidden' ?> mb-5 flex items-center justify-center gap-2 py-3 px-4 rounded-2xl text-sm font-semibold text-gray-700 animate-fade-up" style="background:#F3E6F5;border:1px solid #C7A1C9;">
        <span id="selectedEmoji" class="text-xl"><?= $preselected ? ['','😭','😞','😐','😊','🤩'][$preselected] : '' ?></span>
        <span>Anda memilih: <strong id="selectedLabel" style="color:#5B005F;"><?= $preselected ? ['','Sangat Tidak Puas','Tidak Puas','Biasa Saja','Puas','Sangat Puas'][$preselected] : '' ?></strong></span>
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" style="color:#10B981;">
          <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
        </svg>
      </div>

      <button type="submit" id="csatSubmit" <?= $preselected ? '' : 'disabled' ?>
        class="btn-primary w-full py-3.5 rounded-2xl font-semibold text-base text-white flex items-center justify-center gap-2 shadow-md <?= $preselected ? '' : 'opacity-40 cursor-not-allowed' ?>">
        Lanjutkan ke NPS
        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
